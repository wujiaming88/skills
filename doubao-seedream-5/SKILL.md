...
从用户需求提取用途、主体、构图/比例、风格、准确文字、参考图分工，以及需要修改和保持的部分。

缺少影响结果的关键信息才澄清：只问 1–2 个影响执行路径、资产引用、交付范围或事实准确性的问题，用选择题；关键信息缺 1 项按默认值直接执行并在交付时说明，缺 2 项及以上必须先澄清；澄清最多 1 轮；用户说“你来定”“直接生成”时跳过。

张数按需求定：单点修改 1 张；多方向对比 2–3 张且方向须有明显差异；系列、套图、多平台或多比例适配 3–4 张；需要超过 10 张时先出代表性的一批，确认方向后再继续。张数与一致性冲突时优先减少张数。

用户说“基于刚才那张”“继续做”“延展上一版”时，先确认指向的是哪一张，再按 `edit` 处理；只把明确指向的图作为输入，不把上一轮所有图都传入。
...
阅读 [意图与提示词指南](references/intent-and-prompts.md)：意图判定、接续创作、核心资产、事实与文案边界、提示词结构与具体度策略。画面骨架与阅读顺序见 [构图决策](references/composition.md)，风格、色彩与完成度收尾见 [美学与完成度](references/aesthetics.md)；按需读取，不一次加载全部参考。需要精确尺寸、完整 JSON 或排查参数冲突时阅读 [API 能力与约束](references/api.md)。[命令示例](references/examples.md) 提供可直接调整的调用方式。
